...
基线：commit `260c3f1`
状态：**C1、H2、H3、H4、M7、M8、M9、M12、M14、L18、L21 已修复**（见文末「本轮实施记录」）；M11、M17、H5、L19 未修复
...
| H5 | 文档漂移 | `README.md:73` | High | 0.25 | 原文称「Ubuntu ≥ 24.04 的 root 在 p2，p1 为 EFI 分区」。实测 noble 镜像 p1 的类型 GUID 为 `0fc63daf-…`（Linux filesystem，2559 MiB，即 root），EFI 是 p15，p16 是 Linux extended boot。该段与 `scripts/customize-image.sh:74-75` 的注释（「root on the last one」）互相矛盾，且两句都不对：noble 的 root 在 p1，最后一个分区是 `/boot`。 | 把第 73 行改为对实测布局的描述：Ubuntu ≥24.04 为 p1 root、p15 EFI、p16 `/boot`；同时修正 `scripts/customize-image.sh:74-75` 的注释。 |
| M6 | 测试 | `test/customize-rootfs.bats`（全部用例） | Medium | 3 | 7 个用例全部通过（`bats test` 输出 `1..7` 且全部 ok），但覆盖对象只有 `customize-rootfs.sh` 的文件编辑函数。风险最高的 `scripts/customize-image.sh`（152 行：NBD 挂载、分区探测、growpart、bind mount、trap 拆解）零测试——C1 这个缺陷正落在这段无测试代码里。`install_packages`、`configure_system`、`cleanup_rootfs` 同样无用例。 | 把分区探测抽成可由 fake `blkid`/`lsblk` 输出驱动的函数并加 bats 用例（C1 的断言也应在此实现）；`configure_system` 的纯文件部分（时区、GRUB、serial-getty、NTP、sysctl 安装）可像现有用例一样用 `ROOT` 覆盖测试。 |
...
| M11 | 文档漂移 | `README.md:29` | Medium | 0.1 | README 声称镜像里有「root 自定义 PS1 提示符」，但提交 `260c3f1` 已删除该功能，提交信息写明「drop the custom root PS1/.profile tweak; stock prompt is fine.」，README 未同步。 | 删除 `README.md:29` 该行。 |
...
| M15 | 安全 | `scripts/customize-rootfs.sh:49-61`、`:103`、`README.md:28` | Medium | 0.5 | 镜像默认开启 root 密码登录：`disable_root: false`、`ssh_pwauth: true`、`lock_passwd: False`、`PermitRootLogin yes`，密码由 PVE cloud-init 下发。这是 PVE 模板场景的有意设计（注释已说明），但产物发布到公开 Release，任何下载者都能得到一套「root 可直接密码登录」的镜像，仓库没有任何一处提示这个风险。 | 在 `README.md` 的「镜像定制内容」一节加一条明确的安全提示；更稳妥的做法是在镜像内放置首次登录提示，或把 `PermitRootLogin` 改为 `prohibit-password` 交由使用者在 PVE 侧按需覆盖。 |
...
| M17 | 错误处理 | `scripts/customize-image.sh:69-70` | Medium | 0.5 | `partprobe "$DISKDEV" \|\| true` 之后跟一个 `sleep 2`，注释自述为「let udev settle」。这是用固定等待替代就绪检查：慢机器上 2 秒可能不够，快机器上白等 2 秒。 | 替换为轮询：在超时时间内循环检查期望的分区设备节点是否出现（`udevadm settle` 或轮询 `blkid` 输出），超时后报错退出。 |
...
| L19 | 文档 | `README.md:65` | Low | 0.1 | 原文称构建列表在 `build-cloud-image.yml` 的 `strategy.matrix.include` 中，实际矩阵来源是 `config/images.yaml`，workflow 第 66 行只是 `fromJSON(needs.generate-matrix.outputs.matrix)`。 | 把第 65 行改为指向 `config/images.yaml`。 |
...
实施日期：2026-09-18。工作树状态：已修改但未提交（`git status` 见下），未创建任何提交。
