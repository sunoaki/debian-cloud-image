...
基于 GitHub Actions 自动构建适用于 **Proxmox VE (PVE)** 的 Debian Cloud Image（12 / 13），并把所有版本合并为一个**按日期命名**的 GitHub Release，方便 PVE 侧自动下载。构建配置已矩阵化，后续可以很容易地扩展 Ubuntu Cloud Image。

## 构建流程

1. 从官方源下载 genericcloud 镜像（Debian 用 SHA512 校验）
...
- 关闭 cloud-init 启动时重新生成 apt 源；apt 源去掉 `deb-src`（DEB822 格式）
...
gh release download "$TAG" --pattern 'debian-13-*.qcow2'
```
...
在 `.github/workflows/build-cloud-image.yml` 的 `strategy.matrix.include` 中新增一行即可，例如：
...
产物会自动变为 `ubuntu-24.04-pve-sunoaki+YYYYMMDD.qcow2` 并合并进同一个 Release。
