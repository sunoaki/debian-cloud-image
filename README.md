...
- **预装软件**：`qemu-guest-agent spice-vdagent aria2 net-tools iputils-ping iputils-arping iputils-tracepath mtr-tiny dnsutils sudo bash-completion unzip wget curl nano most screen less vim bzip2 lldpd htop zstd tmux`
...
构建列表在 `config/images.yaml` 中（workflow 读取该文件生成矩阵），目前包含：
